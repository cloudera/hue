      ...
      for dn, data in result_data:
        ...
        if dn is not None:
          ...
          if group_member_attr in data and group_member_attr not in 'memberUid':
            ...
            LOG.debug('Skipping import of non-posix users since group_member_attr is memberUid or group did not contain any members')
            ldap_info['members'] = []

          if 'posixGroup' in data['objectClass'] and 'memberUid' in data:
            ldap_info['posix_members'] = data['memberUid']
          else:
            LOG.debug('Skipping import of posix users.  posixGroup not an objectClass or  no memberUids found')

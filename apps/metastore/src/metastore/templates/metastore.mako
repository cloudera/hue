                ...
                <!-- ko with: source -->
                  <!-- ko with: namespace -->
                    <!-- ko with: database -->
                      ...
                      % endif
                      % if has_write_access:
                        <a href="javascript: void(0);" class="btn btn-default" data-bind="click: showImportData, visible: $root.source().type !== 'sql' && tableDetails() && !catalogEntry.isView()" title="${_('Import Data')}"><i class="fa fa-upload fa-fw"></i> ${_('Import')}</a>
                      % endif
                      % if has_write_access:

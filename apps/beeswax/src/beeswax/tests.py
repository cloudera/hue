...
INFO  : OK"""
    ...
    assert jobs and jobs[0]['job_id'] == 'hive_20191029132605_17883ebe-d3d5-41bf-a1e9-01cf207a3c6b'
    assert jobs and jobs[0]['started'] is True
    assert jobs and jobs[0]['finished'] is True

    text = """INFO  : Compiling command(queryId=hive_20191029132605_17883ebe-d3d5-41bf-a1e9-01cf207a3c6a): select 1
INFO  : OK"""
    jobs = parse_out_queries(text, engine='tez', with_state=True)
    assert jobs and jobs[0]['job_id'] == 'hive_20191029132605_17883ebe-d3d5-41bf-a1e9-01cf207a3c6a'
    assert jobs and jobs[0]['started'] is False
    assert jobs and jobs[0]['finished'] is False
  ...
  def test_install_examples(self):
    with patch('beeswax.management.commands.beeswax_install_examples.Command') as Command:
      ...
      assert 'POST request is required.' in json.loads(resp.content)['message']
...
class TestBeeswaxWithHadoop(BeeswaxSampleProvider):
  ...
  def _verify_query_state(self, state, *extra_states):
    ...
    assert last_state in (state,) + extra_states
    return history.id
  ...
  def test_query_with_error(self):
    ...
    assert "AlreadyExistsException" in content.get('message'), content
...
""")
    ...
    assert [['0'], ['1'], ['4'], ['9']] == content["results"][0:4]
  ...
  def test_lazy_query_status_update(self):
    ...
    resp = self.client.get('/beeswax/query_history')
    history = resp.context[0]['page'].object_list[0]
    assert history.last_state == beeswax.models.QueryHistory.STATE.expired.value

  def test_basic_flow(self):
    # Minimal server operation
    databases = self.db.get_databases()
    assert 'default' in databases, databases
    assert self.db_name in databases, databases
    assert '%s_other' % self.db_name in databases, databases
    ...
    assert [0, 255, 32640] == content["results"][0], content["results"][0]
    assert ['INT_TYPE', 'INT_TYPE', 'BIGINT_TYPE'] == [col['type'] for col in content["columns"]]
    self._verify_query_state(beeswax.models.QueryHistory.STATE.available.value)
    ...
    assert [99, u'0x63'] in content['results'], content['results']
    assert content["has_more"]
    response = self.client.get("/beeswax/results/%s/%s?format=json" % (content["id"], content["next_row"]))
    content = json.loads(response.content)
    assert [199, u'0xc7'] in content['results'], content['results']
    response = self.client.get("/beeswax/results/%s/0?format=json" % (content["id"]))
    content = json.loads(response.content)
    assert [99, u'0x63'] in content['results']
    assert 0 == len(content["hadoop_jobs"]), "SELECT * shouldn't have started jobs."
  ...
  def test_query_with_udf(self):
    ...
    for engine in execution_engines:
      ...
      assert [2.0, 4.0] == content["results"][0]
      log = content['log']

      assert search_log_line('Completed executing command', log), log
      # Test job extraction while we're at it
      assert 1 == len(parse_out_jobs(log, engine)), "Should have started 1 job and extracted it."
  ...
  def test_query_with_remote_udf(self):
    ...
    response = wait_for_query_to_finish(self.client, response, max=60.0)
    assert ["64"] == response.context[0]["results"][0]
  ...
  def test_query_with_simple_errors(self):
    ...
    assert "ParseException line" in json.loads(resp.content)["message"]
  ...
  def test_sync_query_error(self):
    ...
    try:
      ...
      assert bex.errorCode == 40000
      assert bex.SQLState == "42000"
  ...
  def test_fetch_configuration(self):
    class MockClient(object):
      ...
      def fetch(self, query_id, start_over, fetch_size):
        assert self.support_start_over == start_over
  ...
  def test_parameterization(self):
    ...
    assert 'watch_url' in content, content
    ...
    assert "SELECT foo FROM test WHERE foo='1' and bar='2'" == query_history.query
    ...
    content = json.loads(response.content)
    assert "FAILED: ParseException" in content.get('message'), content
    ...
    assert 'watch_url' in content, content
    query_history = QueryHistory.get(content['id'])
    assert other_db == query_history.design.get_design().query['database']

  def test_explain_query(self):
    c = self.client
    response = _make_query(c, "SELECT KITTENS ARE TASTY", submission_type="Explain", database=self.db_name)
    assert "ParseException line" in json.loads(response.content)["message"]
    ...
    assert 'STAGE DEPENDENCIES:' in explanation, explanation
    assert 'STAGE PLANS:' in explanation, explanation
  ...
  def test_query_i18n(self):
    ...
    assert ["200", chr(200)] == response.context[0]["results"][0], "selecting from utf-8 table should get correct result"

    csv = get_csv(self.client, response)
    assert '"200","%s"' % (chr(200).encode('utf-8'),) == csv.split()[1]
    ...
    response = _make_query(self.client, query, wait=True, database=self.db_name)
    assert 'results' in response.context, "selecting from latin1 table should not blow up"
    ...
    assert response.context[0]['table'].parameters['comment'] == self.get_i18n_table_comment()
  ...
  def test_multiple_statements_no_result_set(self):
    ...
    content = json.loads(resp.content)
    assert 'DROP TABLE test_multiple_statements_2' == query_history.get_current_statement(), content
  ...
  def test_multiple_statements_with_result_set(self):
    ...
    assert 'SELECT foo FROM test' == content.get('statement'), content

    resp = wait_for_query_to_finish(self.client, resp, max=30.0)
    content = fetch_query_result_data(self.client, resp)

    assert not content.get('is_finished'), content

    resp = self.client.post(watch_url, {'next': True})
    content = json.loads(resp.content)
    assert 'SELECT count(*) FROM test' == content.get('statement'), content
  ...
  def test_multiple_statements_various_queries(self):
    ...
    content = json.loads(resp.content)
    assert 'CREATE TABLE test_multiple_statements_2 (a int)' == content.get('statement'), content
    ...
    content = fetch_query_result_data(self.client, resp)
    assert content.get('is_finished'), content
  ...
  def test_multiple_statements_with_next_button(self):
    ...
    content = json.loads(resp.content)
    watch_url = content['watch_url']
    assert 'show tables' == content.get('statement'), content

    resp = wait_for_query_to_finish(self.client, resp, max=30.0)
    content = fetch_query_result_data(self.client, resp)
    assert [u'test'] in content.get('results'), content

    # Next statement
    resp = self.client.post(watch_url, {'next': True, 'query-query': hql})
    content = json.loads(resp.content)
    assert 'select * from test' == content.get('statement'), content

    resp = wait_for_query_to_finish(self.client, resp, max=30.0)
    content = fetch_query_result_data(self.client, resp)
    assert [0, u'0x0'] in content.get('results'), content
  ...
  def test_multiple_statements_with_params(self):
    ...
    content = json.loads(resp.content)
    watch_url = content['watch_url']
    assert 'select ${x} from test' == content.get('statement'), content
    ...
    resp = self.client.post(watch_url, {'next': True, 'query-query': hql})
    content = json.loads(resp.content)
    assert 'select ${y} from test' == content.get('statement'), content
  ...
  def test_multiple_statements_with_error(self):
    ...
    resp = self.client.post(watch_url, {'next': True, 'query-query': hql})
    content = json.loads(resp.content)
    assert 'Error while compiling statement' in content.get('message'), content
  ...
  def test_query_done_cb(self):
    ...
    assert 'email_notify is false' in response.content, response.content
    ...
    response = self.client.get('/beeswax/query_cb/done/%s' % query_history.server_id,)
    assert 'sent' in response.content, response.content

    response = self.client.get('/beeswax/query_cb/done/blahblahblah')
    assert 'QueryHistory matching query does not exist' in response.content, response.content
  ...
  def test_data_export(self):
    ...
    assert len(sheet_data) == 257, sheet_data
    ...
    try:
      ...
      assert len(sheet_data) == 6, sheet_data
    finally:
      finish()
  ...
  def test_data_upload(self):
    ...
    assert self.cluster.fs.exists(csv_file)
  ...
  def test_designs(self):
    ...
    assert 'query' in resp.context[0]._data, resp.context
    assert design == resp.context[0]._data['design'], resp.context
    ...
    assert 'query' in resp.context[0]._data, resp.context
    assert resp.context[0]._data['query'] is not None, resp.context
    assert 'design' in resp.context[0]._data, resp.context
    assert resp.context[0]._data['design'] is not None, resp.context

    resp = cli.get(reverse('beeswax:api_fetch_saved_design', kwargs={'design_id': design.id}))
    content = json.loads(resp.content)
    assert 'SELECT bogus FROM test' in content['design']['query'], content
    ...
    assert nplus_designs == n_designs, 'Auto design should not show up in list_designs'
    ...
    assert 'design_id' in content, content

    resp = cli.get('/beeswax/list_designs')
    assert 'rubbish' in resp.content, resp.content
    nplusplus_designs = len(resp.context[0]._data['page'].object_list)
    assert nplusplus_designs > nplus_designs

    # Retrieve that design and check correct DB is selected
    design = beeswax.models.SavedQuery.objects.filter(name='rubbish')[0]
    resp = cli.get(reverse('beeswax:api_fetch_saved_design', kwargs={'design_id': design.id}))
    content = json.loads(resp.content)
    assert query in content['design']['query'], content
    assert '' == content['design']['desc'], content
    assert other_db == content['design']['database'], content
    ...
    assert 'Delete design(s)' in resp.content, resp.content
    ...
    def do_view(param):
      ...
      assert len(resp.context[0]['page'].object_list) >= 0     # Make the query run
      return resp
    ...
    assert 'rubbish' in resp.content
    ...
    try:
      ...
      assert 'client query 1' in resp.content, resp.content
      assert 'client query 2' in resp.content, resp.content
    finally:
      finish()
    ...
    try:
      resp = client_me.get('/beeswax/list_designs')
      assert 'client query 1' in resp.content
      assert 'client query 2' in resp.content
    finally:
      finish()
      client_me.logout()
    ...
    try:
      resp = client_not_me.get('/beeswax/list_designs')
      assert 'client query 1' not in resp.content
      assert 'client query 2' not in resp.content
    finally:
      finish()
  ...
  def test_my_queries(self):
    ...
    assert 'my rubbish kuery' in resp.content, resp.content
    assert 'Even More Bogus Junk' in resp.content
    ...
    assert 'my rubbish kuery' not in resp.content
    assert 'Even More Bogus Junk' not in resp.content
    client_not_me.logout()
  ...
  def test_save_results_to_dir(self):
    ...
    def save_and_verify(select_resp, target_dir, verify=True):
      ...
      if verify:
        ...
        assert 256 == len(data_buf.strip().split('\n'))
        assert '255' in data_buf
    ...
    assert 'Directory already exists' in resp.content, resp.content
    ...
    assert 'File Browser' in resp.content, resp.content
  ...
  def test_save_results_to_file(self):
    ...
    def save_and_verify(select_resp, target_file, overwrite=True, verify=True):
      ...
      # Check that data is right
      if verify:
        assert self.cluster.fs.exists(target_file)
        assert self.cluster.fs.isfile(target_file)
        data_buf = ""
    ...
    assert '-3' in resp.content, resp.content
    assert 'already exists' in resp.content
  ...
  def test_save_results_to_tbl(self):
    ...
    def save_and_verify(select_resp, target_tbl):
      ...
      content = fetch_query_result_data(self.client, resp)
      for i in range(90):
        assert [i, '0x%x' % (i,)] == content['results'][i]
  ...
  def test_install_examples(self):
    assert not beeswax.models.MetaInstall.get().installed_example
    ...
    data = json.loads(resp.content)
    assert 'sample_08' in data['table_names']
    assert 'sample_07' in data['table_names']
    assert 'customers' in data['table_names']
    ...
    if USE_NEW_EDITOR.get():
      ...
      assert 'examples' in doc_names, data
      ...
      assert 'Sample: Job loss' in doc_names, data
      assert 'Sample: Salary growth' in doc_names, data
      assert 'Sample: Top salary' in doc_names, data
      assert 'Sample: Customers' in doc_names, data
    else:
      # New designs exists
      resp = self.client.get('/beeswax/list_designs')
      assert 'Sample: Job loss' in resp.content, resp.content
      assert 'Sample: Salary growth' in resp.content
      assert 'Sample: Top salary' in resp.content
      assert 'Sample: Customers' in resp.content

      # Now install it a second time, and no error
      resp = self.client.post('/beeswax/install_examples', {'db_name': self.db_name})
      assert 0 == json.loads(resp.content)['status']
      assert '' == json.loads(resp.content)['message']
  ...
  def test_create_table_generation(self):
    ...
    assert "Field terminator" in resp.content
    # Make a submission
    ...
    assert "my_col" in [col['name'] for col in data['cols']], data

  def test_create_table_timestamp(self):
    # Check form
    response = self.client.get('/beeswax/create/create_table/%s' % self.db_name)
    assert '<option value="timestamp">timestamp</option>' in response.content, response.content
    ...
    flat_rows = sum(rows, [])
    assert "NULL" in flat_rows, flat_rows
    ...
    rows = json.loads(resp.content)['rows']
    flat_rows = sum(rows, [])
    assert "2012-01-01 10:11:30.0" in flat_rows, flat_rows
  ...
  def test_create_table_dependencies(self):
    ...
    assert resp.context[0]["table_form"].errors["input_format_class"]
    assert resp.context[0]["table_form"].errors["output_format_class"]
    assert resp.context[0]["table_form"].errors["serde_name"]
    assert resp.context[0]["table_form"].errors["serde_properties"]
    assert resp.context[0]["table_form"].errors["serde_properties"]

    assert resp.context[0]["columns_form"].forms[0].errors["map_key_type"]
    assert resp.context[0]["columns_form"].forms[0].errors["map_value_type"]
  ...
  def test_create_table_import(self):
    ...
    assert resp.context[0]['fields_list'] == RAW_FIELDS
    ...
    assert len(resp.context[0]['fields_list'][0]) == 4
    ...
    assert len(resp.context[0]['fields_list'][0]) == 3
    ...
    assert resp.context[0]['fields_list'] == [
      ['a', 'b', 'c'],
      ['a,a', 'b,b', 'c,c'],
      ['a,"a', 'b,"b', 'c,"c'],
    ]
    ...
    assert len(resp.context[0]['column_formset'].forms) == 3
    ...
    #
    assert resp.context[0]['action'] == 'watch-redirect'
    on_success_url_load_data = resp.context[0]['on_success_url']
    assert 'auto_load' in on_success_url_load_data, on_success_url_load_data
    ...
    assert '/metastore/table/' in on_success_url_show_table, on_success_url_show_table
    query_history = resp.context[0]['query_history']
    ...
    assert len(cols) == 3
    assert [col.name for col in cols] == ['col_a', 'col_b', 'col_c']
    resp = self.client.get(reverse('beeswax:get_sample_data', kwargs={'database': self.db_name, 'table': 'test_create_import'}))
    rows = json.loads(resp.content)['rows']
    flat_rows = sum(rows, [])
    assert "nada" in flat_rows, flat_rows
    assert "sp ace" in flat_rows, flat_rows
  ...
  def test_select_invalid_data(self):
    ...
    resp = self.client.get("/beeswax/results/%s/0?format=json" % history_id)
    content = json.loads(resp.content)
    assert [[1.0], [2.0], [3.0], [u'NaN'], [u'NULL'], [u'NULL'], [u'NULL']] == content['results']
  ...
  def test_create_database(self):
    ...
    try:
      ...
      assert db_name in resp.context[0]["databases"], resp
      ...
      resp = wait_for_query_to_finish(self.client, resp, max=180.0)
      resp = self.client.get("/metastore/databases/")
      assert db_name_accent in resp.context[0]['databases'], resp
    finally:
  ...
  def test_select_query_server(self):
    ...
    assert 'beeswax' == history.server_name
    assert history.server_host in [HIVE_SERVER_HOST.get(), 'localhost']

    query_server = history.get_query_server_config()
    assert 'beeswax' == query_server['server_name']

    # NOTE: The history server is typically on a different server when live
    # cluster testing.
    if not is_live_cluster():
      assert get_localhost_name() == query_server['server_host']

    assert 'hiveserver2' == query_server['server_type']
    assert query_server['principal'] is None, query_server['principal']  # No default hive/HOST_@TEST.COM so far

  def test_select_multi_db(self):
    response = _make_query(self.client, 'SELECT * FROM test LIMIT 5', local=False, database=self.db_name)
    response = wait_for_query_to_finish(self.client, response)
    content = fetch_query_result_data(self.client, response)
    assert [0, u'0x0'] in content['results'], content

    response = _make_query(self.client, 'SHOW TABLES', local=False, database='%s_other' % self.db_name)
    response = wait_for_query_to_finish(self.client, response)
    content = fetch_query_result_data(self.client, response)
    assert 'tab_name' in content['columns'][0]['name'], content

    response = _make_query(self.client, 'SELECT * FROM test LIMIT 5', local=False, database='not_there')
    content = json.loads(response.content)
    assert -1 == content.get('status'), content
  ...
  def test_get_table_sample(self):
    ...
    assert 'test.foo' in json_resp['headers'], json_resp
    assert [0, '0x0'] in json_resp['rows'], json_resp
  ...
  def test_get_sample_partitioned(self):
    ...
    try:
      ...
      assert hql == 'SELECT * FROM `%s`.`%s` WHERE %s LIMIT 10' % (self.db_name, table_name, partition_spec)
    finally:
      finish()
  ...
  def test_redacting_queries(self):
    ...
    try:
      ...
      content = json.loads(resp.content)
      assert 'id' in content, 'Query failed: %s' % (content,)

      query_id = content['id']
      history = beeswax.models.QueryHistory.objects.get(pk=query_id)
      assert history.query == expected_query
      assert history.is_redacted
      ...
      history = beeswax.models.QueryHistory.objects.get(pk=query_id)
      assert history.query == expected_query
      assert not history.is_redacted
    finally:
  ...
  def test_analyze_table_and_read_statistics(self):
    ...
    try:
      ...
      assert any([stat for stat in stats if stat['data_type'] == 'numFiles' and stat['comment'] == '1']), resp.content
      ...
      resp = self.client.get(reverse('beeswax:get_table_stats', kwargs={'database': self.db_name, 'table': 'test'}))
      stats = json.loads(resp.content)['stats']
      assert any([stat for stat in stats if stat['data_type'] == 'numRows' and stat['comment'] == '256']), resp.content
  ...
  def test_get_top_terms(self):
    ...
    content = json.loads(resp.content)
    assert 'terms' in content, 'Failed to get terms: %s' % (content,)
    terms = content['terms']

    assert [[255, 1], [254, 1], [253, 1], [252, 1]] == terms[:4]
    ...
    assert [[109, 1], [108, 1], [107, 1], [106, 1]] == terms[:4]
  ...
  def test_beeswax_api_autocomplete(self):
    ...
    assert "nested_table" in [table['name'] for table in tables]
    ...
    assert {'comment': '', 'type': 'array<struct<bar:int,baz:string>>', 'name': 'foo'} == extended_columns[0]
    ...
    json_resp = json.loads(resp.content)
    assert 'error' not in json_resp, 'Failed to autocomplete nested type: %s' % json_resp.get('error')

    assert "array" == json_resp['type']
    assert "item" in json_resp
    assert "struct" == json_resp["item"]["type"]
    ...
    assert "struct" == json_resp['type']
    assert "fields" in json_resp
  ...
  def test_get_indexes(self):
    ...
    json_resp = json.loads(resp.content)
    assert 'headers' in json_resp, json_resp
    assert 'rows' in json_resp, json_resp
    assert 2 == len(json_resp['rows']), json_resp['rows']
  ...
  def test_get_settings(self):
    ...
    try:
      ...
      assert 2 == len(list(json_resp['settings'].items())), json_resp
      assert 'hive.execution.engine' in json_resp['settings']
      assert 'mapreduce.job.queuename' in json_resp['settings']
    finally:
      for reset in resets:
        reset()
  ...
  def test_get_functions(self):
    ...
    json_resp = json.loads(resp.content)
    assert 'functions' in json_resp, json_resp
    assert 'coalesce' in json_resp['functions'], json_resp['functions']

    resp = self.client.get(reverse("beeswax:get_functions"), {'prefix': 'a'})
    json_resp = json.loads(resp.content)
    assert 'functions' in json_resp, json_resp
    assert 'avg' in json_resp['functions'], json_resp['functions']
    assert 'coalesce' not in json_resp['functions'], json_resp['functions']
...
def test_import_gzip_reader():
  ...
  try:
    ...
    lines_joined = '\n'.join(lines)
    assert data[:len(lines_joined)] == lines_joined
  finally:
...
def test_history_page():
  ...
  def do_view(param, n=1):
    ...
    if n == 0:
      if resp.context[0]['page']:
        assert len(resp.context[0]['page'].object_list) == 0
    else:
      assert len(resp.context[0]['page'].object_list) >= n     # Make the query run
    return resp
  ...
  response = do_view('')
  assert {u'q-type': [u'beeswax']} == response.context[0]['filter_params']
...
"""
  ...
  assert [] == parse_out_jobs("nothing to see here")
...
"""
  ...
  assert ["application_1465862139975_0002"] == parse_out_jobs(sample_log, 'tez')
  assert [] == parse_out_jobs("Tez job doesn't exist.", 'tez')
...
def test_hive_site():
  ...
  try:
    ...
    assert beeswax.hive_site.get_conf()['hive.metastore.warehouse.dir'] == u'/abc'
    assert beeswax.hive_site.get_hiveserver2_kerberos_principal('localhost') == 'hs2test/test.com@TEST.COM'
    assert beeswax.hive_site.get_hiveserver2_authentication() == 'NOSASL'
  finally:
    beeswax.hive_site.reset()
...
def test_hive_site_host_pattern_local_host():
  ...
  try:
    ...
    assert beeswax.hive_site.get_conf()['hive.metastore.warehouse.dir'] == u'/abc'
    assert beeswax.hive_site.get_hiveserver2_kerberos_principal(hostname) == 'test/' + socket.getfqdn().lower() + '@TEST.COM'
  finally:
    for finish in reset:
      finish()
    beeswax.hive_site.reset()
...
def test_hive_site_null_hs2krb():
  ...
  try:
    ...
    assert beeswax.hive_site.get_conf()['hive.metastore.warehouse.dir'] == u'/abc'
    assert beeswax.hive_site.get_hiveserver2_kerberos_principal('localhost') is None
    assert beeswax.hive_site.get_hiveserver2_authentication() == 'NOSASL'
  finally:
    beeswax.hive_site.reset()
    if saved is not None:
...
def test_search_log_line():
  logs = """
    ...
    """  # noqa: E501
  assert search_log_line('FAILED: Parse Error', logs)
  ...
  logs = """
    ...
    org.apache.hadoop.hive.ql.parse.ParseException: line 1:31 cannot recognize input near '''' '_this_is_not' 'SQL' in constant
    """  # noqa: E501
  assert not search_log_line('FAILED: Undefined', logs)
  ...
  logs = """
    ...
    """
  assert not search_log_line('FAILED: Parse Error', logs)
...
class TestHiveServer2API(object):
  ...
  def test_parsing_partition_values(self):
    ...
    assert ['2013022516'] == value.values

    value = PartitionValueCompatible(['month=2011-07/dt=2011-07-01/hr=12'], table)
    assert ['2011-07', '2011-07-01', '12'] == value.values
  ...
  def test_hiveserver_table(self):
    ...
    assert 'hdfs://localhost:8020/user/hive/warehouse/test_partitions' == table.path_location
    ...
    assert [PartitionKeyCompatible('baz', 'string', ''),
  ...
  def test_column_format_values_nulls(self):
    def _validate_values_nulls(nulls_array, nulls_string, expected):
      data = [1] * len(expected)
      assert expected == HiveServerTColumnValue2.set_nulls(data, nulls_array)
      assert expected == HiveServerTColumnValue2.set_nulls(data, nulls_string)

    _validate_values_nulls(b'\x00', '\x00', [1, 1, 1])
    _validate_values_nulls(b'\x03', '\x03', [None, None, 1])
    _validate_values_nulls(b't', 't', [1, 1, None, 1, None, None, None, 1])  # 0b1110100
    _validate_values_nulls(b'\xff\xee\x03', '\xff\xee\x03',
                           [None, None, None, None, None, None, None, None, 1, None, None, None, 1, None, None, None, None, None])
    _validate_values_nulls(b'\x41', '\x41', [None, 1, 1, 1, 1, 1, None, 1])
    _validate_values_nulls(b'\x01\x23\x45\x67\x89\xab\xcd\xef', '\x01\x23\x45\x67\x89\xab\xcd\xef', [
    ...
    _validate_values_nulls(b'\xdb\xb6\x00', '\xdb\xb6\x00',
                           [None, None, 1, None, None, 1, None, None, 1, None, None, 1, None, None, 1, None])

    data = [1, 1, 'not_good', 'NaN', None, 'INF', 'INF', 3]
    nulls = 't'  # 0b1110100

    assert ([1, 1, None, 'NaN', None, None, None, 3] ==
  ...
  def test_column_detect_if_values_nulls(self):
    ...
    nulls = ''
    assert data is HiveServerTColumnValue2.set_nulls(data, nulls)
    nulls = '\x00'
    assert data is HiveServerTColumnValue2.set_nulls(data, nulls)
    ...
    nulls = 'aaaa'
    assert data is not HiveServerTColumnValue2.set_nulls(data, nulls)
    nulls = '\x00\x01\x00'
    assert data is not HiveServerTColumnValue2.set_nulls(data, nulls)
  ...
  def test_bits_to_bytes_conversion(self):
    ...
    assert '00000000' == bitstring
    assert [0, 0] == get_bytes_from_bits(bitstring)

    nulls = '\x03'
    bitstring = from_string_to_bits(nulls)
    assert '00000011' == bitstring
    assert [3, 0] == get_bytes_from_bits(bitstring)

    nulls = 't'
    bitstring = from_string_to_bits(nulls)
    assert '01110100' == bitstring
    assert [116, 0] == get_bytes_from_bits(bitstring)

    nulls = '\xff\xee\x03'
    bitstring = from_string_to_bits(nulls)
    assert '111111111110111000000011' == bitstring
    assert [255, 238, 3, 0] == get_bytes_from_bits(bitstring)

    nulls = '\x41'
    bitstring = from_string_to_bits(nulls)
    assert '01000001' == bitstring
    assert [65, 0] == get_bytes_from_bits(bitstring)

    nulls = '\x01\x23\x45\x67\x89\xab\xcd\xef'
    bitstring = from_string_to_bits(nulls)
    assert '0000000100100011010001010110011110001001101010111100110111101111' == bitstring
    assert [1, 35, 69, 103, 137, 171, 205, 239, 0] == get_bytes_from_bits(bitstring)
...
class TestWithMockedServer(object):
  ...
  def test_bulk_query_trash(self):
    ...
    resp = self.client.get('/beeswax/list_designs')
    ids_page_1 = set([query.id for query in resp.context[0]['page'].object_list])
    assert 2 == sum([query_id in ids_page_1 for query_id in ids])
    ...
    resp = self.client.get('/beeswax/list_designs')
    ids_page_1 = set([query.id for query in resp.context[0]['page'].object_list])
    assert 0 == sum([query_id in ids_page_1 for query_id in ids])
    ...
    assert not SavedQuery.objects.filter(id__in=ids).exists()
  ...
  def test_save_design(self):
    ...
    assert 'test_save_design' == saved_design.name
    assert 'test_save_design desc' == saved_design.desc
    assert 'test_save_design' == saved_design.doc.get().name
    assert 'test_save_design desc' == saved_design.doc.get().description
    assert not saved_design.doc.get().is_historic()
    ...
    assert 'test_save_design (new)' == saved_design.name
    assert 'test_save_design desc' == saved_design.desc
    assert 'test_save_design (new)' == saved_design.doc.get().name
    assert 'test_save_design desc' == saved_design.doc.get().description
    assert saved_design.doc.get().is_historic()
    ...
    try:
      ...
      assert False, 'not_me is not allowed'
  ...
  def test_get_history_xss(self):
    ...
    assert sql_escaped in resp.content, resp.content
    if not isinstance(sql, bytes):
      sql = sql.encode('utf-8')
    assert sql not in resp.content, resp.content
  ...
  def test_redact_saved_design(self):
    ...
    try:
      ...
      assert data['query']['query'] == expected_query
      assert design.is_redacted
  ...
  def test_search_designs(self):
    ...
    assert 0 == sum([query_id in ids_page for query_id in page_1])
    assert 1 == sum([query_id in ids_page for query_id in page_2])
  ...
  def test_clear_history(self):
    ...
    assert design_id in design_ids, json_resp
    ...
    json_resp = json.loads(resp.content)
    design_ids = [history['design_id'] for history in json_resp['queries']]
    assert design_id not in design_ids, json_resp
...
class TestDesign(object):
  ...
  def test_hql_resource(self):
    ...
    assert re.match(r'ADD FILE hdfs://([^:]+):(\d+)my_file', statements[0]), statements[0]
    assert re.match(r'ADD FILE hdfs://([^:]+):(\d+)/my_path/my_file', statements[1]), statements[1]
    assert 'ADD FILE s3://host/my_s3_file' == statements[2]
...
def test_hiveserver2_get_security():
  ...
  try:
    ...
    assert principal.startswith('hive/'), principal

    principal = get_query_server_config('impala')['principal']
    assert principal.startswith('impala/'), principal
    ...
    assert (True, 'PLAIN', 'hive', True, 'hue', None) == HiveServerClient(beeswax_query_server, user).get_security()

    # HiveServer2 LDAP passthrough
    beeswax_query_server.update({'auth_username': 'hueabcd', 'auth_password': 'abcd'})
    assert (True, 'PLAIN', 'hive', True, 'hueabcd', 'abcd') == HiveServerClient(beeswax_query_server, user).get_security()
    ...
    assert (True, 'GSSAPI', 'hive', False, 'hue', None) == HiveServerClient(beeswax_query_server, user).get_security()

    hive_site._HIVE_SITE_DICT[hive_site._CNF_HIVESERVER2_AUTHENTICATION] = 'NOSASL'
    beeswax_query_server.update({'use_sasl': False})
    assert (False, 'NOSASL', 'hive', False, 'hue', None) == HiveServerClient(beeswax_query_server, user).get_security()
    ...
    assert (False, 'GSSAPI', 'impala', False, 'hue', None) == HiveServerClient(impala_query_server, user).get_security()
    ...
    impala_query_server.update(default_query_server)
    assert (False, 'GSSAPI', 'impala', True, 'hue', None) == HiveServerClient(impala_query_server, user).get_security()

    impala_query_server.update({'use_sasl': True})
    assert (True, 'GSSAPI', 'impala', True, 'hue', None) == HiveServerClient(impala_query_server, user).get_security()
  finally:
    if prev is not None:
...
def test_metastore_security():
  ...
  try:
    ...
    assert metastore['use_sasl']
    assert 'thrift://darkside-1234:9999' == metastore['thrift_uri']
    assert 'hive/darkside-1234@test.com' == metastore['kerberos_principal']
  finally:
    beeswax.hive_site.reset()
    if saved is not None:
      beeswax.conf.HIVE_CONF_DIR = saved
...
def test_close_queries_flag():
  ...
  try:
    ...
    assert b'closeQuery()' not in resp.content, resp.content
  finally:
    finish()
  ...
  try:
    resp = c.get('/beeswax/execute')
    assert b'closeQuery()' in resp.content, resp.content
  finally:
    finish()
...
def test_auth_pass_through():
  ...
  try:
    assert 'hue' == AUTH_USERNAME.get()
    assert None is AUTH_PASSWORD.get()
  finally:
    for f in finish:
      f()
  ...
  finish.append(AUTH_PASSWORD.set_for_testing(present=False))
  try:
    assert 'deprecated_default_username' == AUTH_USERNAME.get()
    assert 'deprecated_default_password' == AUTH_PASSWORD.get()
  finally:
    for f in finish:
      f()
  ...
  try:
    assert 'default_username' == AUTH_USERNAME.get()
    assert 'default_password' == AUTH_PASSWORD.get()
  finally:
    for f in finish:
      f()
  ...
  try:
    assert 'hive_username' == AUTH_USERNAME.get()
    assert 'hive_password' == AUTH_PASSWORD.get()
  finally:
    for f in finish:
      f()
  ...
  try:
    assert 'default_username' == AUTH_USERNAME.get()
    assert 'my_hue_secret' == AUTH_PASSWORD.get()
  finally:
    for f in finish:
      f()
  ...
  for desktop_kwargs, conf_kwargs, expected in [
    ...
    try:
      assert conf.SSL.VALIDATE.get() == expected, 'desktop:%s conf:%s expected:%s got:%s' % (
        desktop_kwargs, conf_kwargs, expected, conf.SSL.VALIDATE.get())
    finally:
      for reset in resets:
        reset()
...
def test_to_matching_wildcard():
  ...
  assert match_fn(None) == '*'
  assert match_fn('') == '*'
  assert match_fn('*') == '*'
  assert match_fn('test') == '*test*'
  assert match_fn('test*') == '*test*'


def test_apply_natural_sort():
  test_strings = ['test_1', 'test_100', 'test_2', 'test_200']
  assert apply_natural_sort(test_strings) == ['test_1', 'test_2', 'test_100', 'test_200']

  test_dicts = [{'name': 'test_1', 'comment': 'Test'},
                {'name': 'test_100', 'comment': 'Test'},
                {'name': 'test_2', 'comment': 'Test'},
                {'name': 'test_200', 'comment': 'Test'}]
  assert apply_natural_sort(test_dicts, key='name') == [{'name': 'test_1', 'comment': 'Test'},
                                                            {'name': 'test_2', 'comment': 'Test'},
                                                            {'name': 'test_100', 'comment': 'Test'},
                                                            {'name': 'test_200', 'comment': 'Test'}]
...
def test_hiveserver2_jdbc_url():
  ...
  try:
    ...
    assert url == 'jdbc:hive2://' + hostname + ':10000/default'

    beeswax.conf.HIVE_SERVER_HOST.set_for_testing('server-with-ssl-enabled.com')
    beeswax.conf.HIVE_SERVER_PORT.set_for_testing('10000')
    url = hiveserver2_jdbc_url()
    assert url == 'jdbc:hive2://server-with-ssl-enabled.com:10000/default'
    ...
    url = hiveserver2_jdbc_url()
    assert url == 'jdbc:hive2://server-with-ssl-enabled.com:10001/default;transportMode=http;httpPath=cliservice'
  finally:
    beeswax.hive_site.reset()
    hadoop.ssl_client_site.reset()
    for reset in resets:

...
<script type="text/html" id="hbase">
  <div class="control-group" data-bind="css: {'error': hbase.cluster.errors().length > 0}">
    <label for="name" class="control-label">${_("Database")}</label>
    <div class="controls">
      <select data-bind="options: hbase.clusters, optionsText: function(cluster) {return cluster.name()}, value: hbase.cluster" name="type"></select>
    </div>
  </div>

  <!-- ko if: hive.database() -->
  <div class="control-group" data-bind="css: {'error': hbase.table.errors().length > 0}">
    <label for="name" class="control-label">${_("Table")}</label>
    <div class="controls">
      <select data-bind="options: hbase.cluster().tables, optionsText: function(table) {return table.name()}, value: hbase.table" name="type"></select>
    </div>
  </div>
  <!-- /ko -->
</script>
...
<script type="text/html" id="hive">
  ...
  <!-- ko if: hive.database() -->
  <div class="control-group" data-bind="css: {'error': hive.table.errors().length > 0}">
    <label for="name" class="control-label">${_("Table")}</label>
    <div class="controls">
      <select data-bind="options: hive.database().tables, optionsText: function(table) {return table.name()}, value: hive.table" name="type"></select>
    </div>
  </div>
  <!-- /ko -->
</script>
...
<script type="text/html" id="hbase-mapping">
...
function validateNamePage() {
  ...
  if (vm.source() == 'file') {
    ...
    vm.wizard.getPage('name').next('hbase-mapping');
  } else if (vm.source() == 'hive') {
    vm.inferFields($.map(vm.hive.table().columns(), function(column) { return [[column, 'string']]; }));

...
class TestUploadFileAPI:
  ...
  @patch("filebrowser.api.operations.upload_file")
  def test_api_upload_success(self, mock_operations_upload):
    mock_operations_upload.return_value = {"uploaded_file_stats": {"path": "/tmp/test/test.csv", "size": 14, "type": "file"}}

    test_file = SimpleUploadedFile("test.csv", b"test,content\n")
    request = self.factory.post(reverse("api:storage_upload_file"), {"file": test_file, "destination_path": "/tmp/test", "overwrite": False}, format="multipart")
    request.user = Mock(username="testuser")

    response = upload_file(request)

    assert response.status_code == 201  # HTTP_201_CREATED
    assert response.data["uploaded_file_stats"]["path"] == "/tmp/test/test.csv"

  @patch("filebrowser.api.operations.upload_file")
  def test_api_file_not_found(self, mock_operations_upload):
    mock_operations_upload.side_effect = FileNotFoundError("Destination not found")

    test_file = SimpleUploadedFile("test.csv", b"content")
    request = self.factory.post(reverse("api:storage_upload_file"), {"file": test_file, "destination_path": "/nonexistent"}, format="multipart")
    request.user = Mock(username="testuser")

    response = upload_file(request)

    assert response.status_code == 404  # HTTP_404_NOT_FOUND
    assert "Destination not found" in response.data["error"]

  @patch("filebrowser.api.operations.upload_file")
  def test_api_file_exists(self, mock_operations_upload):
    mock_operations_upload.side_effect = FileExistsError("File already exists")

    test_file = SimpleUploadedFile("test.csv", b"content")
    request = self.factory.post(reverse("api:storage_upload_file"), {"file": test_file, "destination_path": "/tmp/test"}, format="multipart")
    request.user = Mock(username="testuser")

    response = upload_file(request)

    assert response.status_code == 409  # HTTP_409_CONFLICT
    assert "File already exists" in response.data["error"]

  @patch("filebrowser.api.operations.upload_file")
  def test_api_permission_error(self, mock_operations_upload):
    mock_operations_upload.side_effect = PermissionError("Permission denied")

    test_file = SimpleUploadedFile("test.csv", b"content")
    request = self.factory.post(reverse("api:storage_upload_file"), {"file": test_file, "destination_path": "/protected"}, format="multipart")
    request.user = Mock(username="testuser")

    response = upload_file(request)

    assert response.status_code == 403  # HTTP_403_FORBIDDEN
    assert "Permission denied" in response.data["error"]

  def test_api_invalid_data(self):
    # Missing file
    request = self.factory.post(reverse("api:storage_upload_file"), {"destination_path": "/tmp/test"}, format="multipart")
    request.user = Mock(username="testuser")
  ...
  def test_api_restricted_file_type(self):
    ...
    try:
      ...
      assert response.status_code == 400  # HTTP_400_BAD_REQUEST
      assert "is restricted" in str(response.data)
    finally:
      for reset in resets:
        reset()

  @patch("filebrowser.api.operations.upload_file")
  @patch("filebrowser.api.LOG")
  def test_api_generic_exception(self, mock_log, mock_operations_upload):
    mock_operations_upload.side_effect = Exception("Unexpected error")
    ...
    assert response.status_code == 500  # HTTP_500_INTERNAL_SERVER_ERROR
    assert response.data["error"] == "An error occurred while uploading the file."

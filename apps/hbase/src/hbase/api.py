...
from hbase.hbase_site import get_server_principal, get_server_authentication, is_using_thrift_ssl, \
  is_using_thrift_http, get_thrift_transport
...
class HbaseApi(object):
  ...
  def createTable(self, cluster, tableName, columns):
    client = self.connectCluster(cluster)
    client.createTable(tableName, [get_thrift_type('ColumnDescriptor')(**column['properties'])
                                   for column in columns], doas=self.user.username)
    return "%s successfully created" % tableName

  def getTableList(self, cluster):
    client = self.connectCluster(cluster)
    return [{'name': name, 'enabled': client.isTableEnabled(name, doas=self.user.username)}
  ...
  def getAutocompleteRows(self, cluster, tableName, numRows, query):
    ...
    try:
      client = self.connectCluster(cluster)
      scan = get_thrift_type('TScan')(startRow=query, stopRow=None, timestamp=None, columns=[],
                                      caching=None, filterString="PrefixFilter('" + query + "') AND ColumnPaginationFilter(1,0)",
                                      batchSize=None)
  ...
  def getRowsFull(self, cluster, tableName, startRowKey, numRows):
    client = self.connectCluster(cluster)
    return self.getRows(cluster, tableName,
                        [smart_str(column) for column in client.getColumnDescriptors(tableName, doas=self.user.username)],
                        smart_str(startRowKey), numRows)
  ...
  def getRowPartial(self, cluster, tableName, rowKey, offset, number):
    client = self.connectCluster(cluster)
    scan = get_thrift_type('TScan')(startRow=rowKey, stopRow=None, timestamp=None, columns=[],
                                    caching=None, filterString="PrefixFilter('%s') AND ColumnPaginationFilter(%i, %i)"
                                                               % (rowKey, number, offset), batchSize=None)
    scanner = client.scannerOpenWithScan(tableName, scan, None, doas=self.user.username)
  ...
  def putUpload(self, cluster, tableName, row, column, value):
    client = self.connectCluster(cluster)
    Mutation = get_thrift_type('Mutation')
    return client.mutateRow(tableName, smart_str(row),
                            [Mutation(column=smart_str(column), value=value.file.read(value.size))],
                            None, doas=self.user.username)
  ...
  def getRowQuerySet(self, cluster, tableName, columns, queries):
    ...
    for query in queries:
      ...
      scan = get_thrift_type('TScan')(startRow=smart_str(query['row_key']),
                                      stopRow=None, timestamp=None, columns=scan_columns,
                                      caching=None, filterString=filterstring, batchSize=None)
      scanner = client.scannerOpenWithScan(tableName, scan, None, doas=self.user.username)

...
var WorkflowEditorViewModel = function (layout_json, workflow_json, credentials_json, workflow_properties_json, subworkflows_json, can_edit_json) {
  ...
  contextCatalog.getNamespaces({ connector: { type: 'oozie' } }).done(function (context) { self.availableNamespaces(context.namespaces) });
  contextCatalog.getComputes({ connector: { type: 'oozie' } }).done(self.availableComputes);

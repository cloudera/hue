...
export default function doTransform(data, query) {
...
function addCounters(nodes, query) {
  if(query && query.counters) {
    var hiveCounters = query.counters.find(counterGroup => counterGroup.counterGroupName === "HIVE");

    if(hiveCounters) {
      var counterHash = {};
      hiveCounters.counters.forEach(function (counter) {
        counterHash[counter.counterName] = counter.counterValue;
      });
      nodes.forEach(function (node) {
        node.counters = {
          recordsOut: counterHash["RECORDS_OUT_OPERATOR_" + node["OperatorId:"]]
        };
      });
    }
  }

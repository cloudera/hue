...
import {
  ...
  getEdgesWithCorrectedUnion,
  getAdjustedVerticesAndEdges
} from './processor';

export default function doTransform(data: any, query: any): any {
  const tez = getTezPlan(data);
  const fetch = getFetchPlan(data);

  let vertices = [...(tez ? getTezVertices(tez) : []), getFetchVertex(fetch)];
...
function addCounters(nodes: any[], query: any): any {
  if (query && query.counters) {
    const hiveCounters = query.counters.find(
      (counterGroup: any) => counterGroup.counterGroupName === 'HIVE'
    );

    if (hiveCounters) {
      const counterHash = {};
      hiveCounters.counters.forEach((counter: any) => {
...
function getVerticesWithIndexOfChildren(vertices: any[]): any[] {
  const verticesWithIndexX = vertices.map((cVertex: any) =>
    Object.assign({}, cVertex, {
      _children: doGetChildrenWithIndexX(cVertex._children, 0)
    })
  );

  const verticesWithIndexY = verticesWithIndexX.map((cVertex: any) =>
    Object.assign({}, cVertex, {
...
function doGetChildrenWithIndexX(children: any[], cIndex: number): any {
  return children.map((cChild: any) =>
    Object.assign({}, cChild, {
      _indexX: cIndex,
...
function doGetChildrenWithIndexY(children: any[], cIndex: number): any {
  return children.map((cChild: any, index: number) =>
    Object.assign({}, cChild, {
...
function getTezPlan(data: any): any {
  const stages = data['STAGE PLANS'];
  const tezStageKey = Object.keys(stages).find((cStageKey: string) =>
    stages[cStageKey].hasOwnProperty('Tez')
  );
  return stages[tezStageKey] && stages[tezStageKey]['Tez'];
}

function getFetchPlan(data: any): any {
  const stages = data['STAGE PLANS'];
  const fetchStageKey = Object.keys(stages).find((cStageKey: string) =>
    stages[cStageKey].hasOwnProperty('Fetch Operator')
  );
  return stages[fetchStageKey] && stages[fetchStageKey]['Fetch Operator'];
}

function getVertexTree(edges: any): any {
  const rootKey = edges.find((cEdge: any) =>
    edges.every((tcEdge: any) => cEdge._target !== tcEdge._source)
  )._target;
...
function getPrunedTree(node: any, used = {}): any {
  const vertices = node._vertices.filter((cVertex: any) => used[cVertex._vertex] !== true);
  vertices.forEach((cVertex: any) => (used[cVertex._vertex] = true));
  return Object.assign({}, node, {
    _vertices: vertices.map((cVertex: any) => getPrunedTree(cVertex, used))
  });
}

function buildTree(vertexKey: any, edges: any[]): any {
...
function getEdgesWithFetch(tezEdges: any[], vertices: any[]): any[] {
  const rootKeys = tezEdges
...
function getFetchVertex(plan: any): any {
  let children = [];
...
function getTezVertices(plan: any): any {
  const VERTEX_TREE_KEYS = ['Reduce Operator Tree:', 'Map Operator Tree:'];
  const vertexObj = plan['Vertices:'];

  const vertices = Object.keys(vertexObj).map((cVertexKey: string) => {
    const cVertex = vertexObj[cVertexKey];

    const cTreeKey = VERTEX_TREE_KEYS.find(cVertexTreeKey =>
      cVertex.hasOwnProperty(cVertexTreeKey)
    );
    let root = [{ [cVertexKey]: {} }];
    if (cTreeKey) {
      // children available
...
function doHarmonize(nodes: any): any {
  if (Array.isArray(nodes) === false) {
    return doHarmonize([nodes]);
  }

  return nodes.map((cNode: any) => {
...
function getTreeWithOffsetAndHeight(node: any, vertices: any[], connections: any[]): any {
  ...
  const treeWithOffsetY = Object.assign(
    {},
    getTreeWithOffsetYInHiererchy(treeWithIndividualWidth, connections),
    {
      _offsetY: 0
    }
  );
...
function doGetWidthOfNodes(children = []): number {
...
function getTreeWithEffectiveOffsetX(node: any, positionX: number): any {
  const _vertices = node._vertices.map((cVertex: any) =>
    getTreeWithEffectiveOffsetX(cVertex, positionX + node._widthOfSelf)
  );
...
function getTreeWithEffectiveOffsetY(node: any, positionY: number) {
  const _vertices = node._vertices.map((cVertex: any) =>
    getTreeWithEffectiveOffsetY(cVertex, positionY + cVertex._offsetY)
  );
...
function doGetHeightOfNodes(children: any[]) {
  if (children.length > 0) {
...
function getTreeWithCumulativeHeight(node: any, vertices: any[], connections: any[]): any {
  const vertexKey = node._vertex;
  const vertex = vertices.find(cVertex => cVertex._vertex === vertexKey);

  // if does not overlap > add 1
  const vertexNext =
    node._vertices[0] && vertices.find(cVertex => cVertex._vertex === node._vertices[0]._vertex);
  const source = vertexNext && getLastOperatorOf(vertexNext);
  const target = getFirstOperatorOf(vertex);
  const isFirstConnectedToLast = connections.some(
    cConnection =>
      source &&
      target &&
      cConnection._source._uuid === source._uuid &&
      cConnection._target._uuid === target._uuid
  );

  let _height = doGetHeightOfNodes(vertex._children);
  let _vertices = [];
  if (Array.isArray(node._vertices)) {
    _vertices = node._vertices.map((cVertex: any) =>
      getTreeWithCumulativeHeight(cVertex, vertices, connections)
    );
    _height = Math.max(
      _height,
      _vertices.reduce((height, cVertex) => height + cVertex._height, 0)
    );
  }
...
function getTreeWithIndividualWidth(node: any, vertices: any[]): any {
  const vertexKey = node._vertex;
  const vertex = vertices.find(cVertex => cVertex._vertex === vertexKey);
...
function getTreeWithOffsetYInHiererchy(node: any, connections: any[]): any {
  const _vertices = [];
  ...
  for (let index = 0; index < node._vertices.length; index++) {
    ...
    _vertices.push(
      Object.assign({}, getTreeWithOffsetYInHiererchy(cNode, connections), {
        _offsetY: offsetY
      })
    );
...
function doCloneAndOmit(obj: any, keys: string[]): any {
...
function getConnections(vertices: any[], edges: any[]): any[] {
  const connections = [];

  // iterate inside vertices to build connections between children
  vertices.forEach((cVertex: any) => {
  ...
  edges.forEach(cEdge => {
    ...
    const targetOperator =
      findVertexAsInputInNode(targetVertex, cEdge._source) || getFirstOperatorOf(targetVertex);

    const sourceOperatorList = [];
    const targetOperatorList = [];
    const srcNode = {};
    const targNode = {};

    let sourceVertexDef = undefined;
    let targetVertexDef = undefined;
    ...
    if (sourceOperatorList.length && targetOperatorList.length) {
      sourceOperatorList.forEach((item: any) => {
...
function findVertexAsInputInNode(node: any, vertexId: string): boolean {
  ...
  if (Array.isArray(node._groups)) {
    isInputPresent =
      isInputPresent ||
      node._groups.some((cGroupedOperator: any) => {
...
function findAllOperatorsInTargetVertex(node: any, resultsAggregator: any, targetNode: any): any[] {
  const outputOperator = node['OperatorId:'];
  if (outputOperator) {
    resultsAggregator.push(outputOperator);
    targetNode[outputOperator] = node;
  }
  node._children.forEach((cChild: any) =>
    findAllOperatorsInTargetVertex(cChild, resultsAggregator, targetNode)
  );
  if (!node._children) {
...
function getLastOperatorOf(vertex: any): any {
...
function getFirstOperatorOf(vertex: any): any {
  return vertex._children[0];
}

function findOperatorsInNode(node: any, operatorKey: string, resultsAggregator: any): any {
  if (node._operator === operatorKey) {
    return resultsAggregator.push(node);
  }

  node._children.forEach((cChild: any) =>
    findOperatorsInNode(cChild, operatorKey, resultsAggregator)
  );

  return resultsAggregator;
}

function getIntraNodeConnections(node: any): any {
  return node._children.reduce((aggregator: any[], cChild: any) => {
    aggregator.push({
      _source: node,
      _target: cChild
    });
    aggregator.push(...getIntraNodeConnections(cChild));
    return aggregator;
  }, []);
}

function getNodes(vertices: any): any {
  return vertices.reduce(
    (accumulator: any[], cVertex: any) => [
      ...accumulator,
...
function getNodesFromChildren(children: any[]) {
  return children.reduce(
    (accumulator: any[], cChild: any) => [
      ...accumulator,
      cChild,

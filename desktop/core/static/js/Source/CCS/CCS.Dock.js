...
provides: [CCS.Dock]
requires: [ccs-shared/CCS, clientcide/StickyWin.PointyTip, /CCS.User]
script: CCS.Dock.js
...
CCS.Dock = {
	initialize: function(){
		this.element = $('ccs-dock');
		this.statusContent = $('ccs-dock-status-content');
		//show/hide tips when mousing over dock icons; launch the app when clicked
		this.icons = $('ccs-dock-icons').addEvents({
	...
	*/
	showAppTip: function(e, a){
		var component = a.retrieve('CCS-app');
		if (!component) return;
		var name = CCS.Desktop.getBootstrap(component).name;
	...
	*/
	addApp: function(component){
		var appData = CCS.Desktop.getBootstrap(component);
		if (!appData.menu || !appData.menu.img) return;
		var lnk = new Element('a', {
			id: appData.menu.id
		});
		var img = new Element('img', appData.menu.img).addClass('ccs-icon').inject(lnk);
		var dock = $('ccs-dock-icons');
		lnk.inject(dock);
		// Sort the menu.
	...
	*/
	launchApp: function(e, a, launchAnother) {
		...
		var instances = CCS.Desktop.focusComponent(component);
		...
		if (!instances.length || launchAnother) {
			...
			//launch it
			CCS.Desktop.launch(component, a.get('href'), function(){

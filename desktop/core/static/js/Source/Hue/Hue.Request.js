...
provides: [Hue.Request]
requires: [JFrame/JFrame.Request]
script: Hue.Request.js
 ...
 * side to interpose a Hue.Login box. Once the user has logged in, it reposts
	...
	var refactoring = {
		...
		handleLoginRequired: function() {
			...
			if (! loginFormInstance) {
				loginFormInstance = new Hue.Login({
					onSuccess: function() {
		...
		genericErrorAlert: function(data) {
			var errorAlert;
			if (data.message) errorAlert = Hue.error('Hue Error', data.message);
			else errorAlert = Hue.error('Hue Error', "Unknown");
			this.fireEvent('hueErrorPopup', errorAlert);
		}

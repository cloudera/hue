...
/*
---
description: A modal window which asks the user to login.  Called by Hue.Request when the server-side middleware hits a URL for which login is required.
provides: [Hue.Login]
requires: [/Hue.SolidWindow, /ThumbTack, Core/Request.JSON, /Fx.Shake,
  /Hue.User, More/OverText, More/Mask, clientcide/StickyWin.PointyTip, /Hue.Desktop]
script: Hue.Login.js
...
Hue.Login = new Class({
	...
	show: function() {
		new Request.HTML({
			...
			onSuccess: function(tree, elements, html, js) {
				...
				this.window = new Hue.SolidWindow({
				...
				var first = $('hue-flogin-first');
				var pw = $(this.window).getElement('#hue-pwhelp');
				var tip;
				if (first) {
					pw.hide();
	...
	mask: function(){
		$(this.window).addClass('hue-loading');
		try { //some browsers hate this
			this.form.getElement('input.hue-submit').blur();
		} catch(e) {dbug.log(e);}
	},

	//removes the loading state from the UI
	unmask: function(){
		$(this.window).removeClass('hue-loading');
	},
	...
	send: function(){
		...
		var fail = function(){
			...
			Hue.error('Login Error', 'Something seems to be amiss. You can reload and try again or contact us for help.');
			this.unmask();
		}.bind(this);
	...
	//validates the form
	validate: function(){
		return this.form.getElement('input.hue-password').get('value').trim() && 
			this.form.getElement('input.hue-username').get('value').trim();
	},

  //when the form is successful
	//hide the login box and call login
	success: function(user_data){
		if ($('hue-restore-option') && !$('hue-restore-option').getElement('input').get('checked')) Hue.Desktop.resetSession();
		this.tack.detach();
  ...
  //displays the error transition when the credentials are bad
	error: function(){
		this.form.getElement('input.hue-password').set('value', '').fireEvent('change');
		$(this.window).addClass('hue-error').set('shake', {
			link:'ignore'
		}).shake('left', 20);
		this.unmask();
	}

...
AI_INTERFACE = ConfigSection(
  ...
  members=dict(
    ...
    AUTO_FETCH_TABLE_META_LIMIT=Config(
      key='auto_fetch_table_meta_limit',
...
SEMANTIC_SEARCH = ConfigSection(
  ...
  members=dict(
    ...
    MODEL_CACHE_DIR=Config(
      key='model_cache_dir',
      help=_('Local file system directory used for caching the embedding model'),
      default=None,
      type=str),
    TOP_K=Config(
      key='top_k',
      help=_('Number of items with the highest ranking returned by semantic search'),
      default=10,
      type=int),
    CACHE_SIZE=Config(
      key='cache_size',

  ...
  def _get_browser(self):
    ...
    if 'filebrowser' in self.apps and ANALYTIC_DB not in self.cluster_type and fsmanager.is_enabled_and_has_access('hdfs', self.user):
      interpreters.append({
        ...
        'buttonName': _('Browse'),
        'tooltip': _('Files'),
        'page': '/filebrowser/' + (not self.user.is_anonymous() and 'view=' + urllib.parse.quote(self.user.get_home_directory().encode('utf-8'), safe=SAFE_CHARACTERS_URI_COMPONENTS) or '')
      })
    ...
    if 'filebrowser' in self.apps and not IS_EMBEDDED.get() and fsmanager.is_enabled_and_has_access('s3a', self.user):
      interpreters.append({
        ...
        'buttonName': _('Browse'),
        'tooltip': _('S3'),
        'page': '/filebrowser/view=' + urllib.parse.quote('S3A://'.encode('utf-8'), safe=SAFE_CHARACTERS_URI_COMPONENTS)
      })
    ...
    if 'filebrowser' in self.apps and ANALYTIC_DB not in self.cluster_type and fsmanager.is_enabled_and_has_access('adl', self.user):
      interpreters.append({
        ...
        'buttonName': _('Browse'),
        'tooltip': _('ADLS'),
        'page': '/filebrowser/view=' + urllib.parse.quote('adl:/'.encode('utf-8'), safe=SAFE_CHARACTERS_URI_COMPONENTS)
      })

...
def compress_files_in_hdfs(request, file_names, upload_path, archive_name):
  ...
  files = [{"value": upload_path + '/' + urllib.parse.quote(file_name.encode('utf-8'), SAFE_CHARACTERS_URI)} for file_name in file_names]
  ...
  shell_notebook = Notebook(
    ...
    isManaged=True,
    onSuccessUrl='/filebrowser/view=' + urllib.parse.quote(upload_path.encode('utf-8'), safe=SAFE_CHARACTERS_URI_COMPONENTS)
  )

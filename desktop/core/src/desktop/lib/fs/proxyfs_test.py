...
def test_fs_selection():
  ...
  with patch('desktop.lib.fs.ProxyFS._has_access') as _has_access:
    ...
    proxy_fs = ProxyFS({'s3a': wrapper(s3fs), 'hdfs': wrapper(hdfs), 'adl': wrapper(adls), 'abfs': wrapper(abfs), 'gs': wrapper(gs), 'ofs': wrapper(ofs)}, 'hdfs')
...
class TestFsPermissions(object):
  ...
  def test_fs_permissions_regular_user(self):
    ...
    s3fs, adls, hdfs, abfs, gs, ofs = MockFs("s3_access"), MockFs("adls_access"), MockFs(), MockFs("abfs_access"), MockFs("gs_access"), MockFs("ofs_access")
    proxy_fs = ProxyFS({'s3a': wrapper(s3fs), 'hdfs': wrapper(hdfs), 'adl': wrapper(adls), 'abfs': wrapper(abfs), 'gs': wrapper(gs), 'ofs': wrapper(ofs)}, 'hdfs')
    proxy_fs.setuser(user)

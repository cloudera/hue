...
const TEMPLATE = `
  ...
  <div class="jobs-panel history-panel hide" data-bind="visible: historyPanelVisible, style: { 'top' : top, 'left': left }">
    ...
    <!-- ko ifnot: editorViewModel.selectedNotebook() && editorViewModel.selectedNotebook().history().length > 0 -->
      <span data-bind="text: I18n('No task history.'), style: { fontStyle: 'italic' }"></span>
    <!-- /ko -->
    ...
    <div class="notification-history margin-bottom-10">
      ...
      <pre data-bind="visible: result.logs() && result.logs().length > 0, text: result.logs, logScroller: result.logs, logScrollerVisibilityEvent: showLogs, style: { height: '120px', 'overflow-y': 'auto'}" class="logs logs-bigger logs-populated"></pre>
      <!-- /ko -->
      <!-- /ko -->
      <!-- /ko -->
    </div>

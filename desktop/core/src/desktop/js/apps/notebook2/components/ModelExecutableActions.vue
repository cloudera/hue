...
<template>
  <div class="snippet-execute-actions">
    ...
    <hue-button
      ...
      @click="execute"
    >
      <i class="fa fa-play fa-fw" /> {{ I18n('Predict') }}
    </hue-button>
    ...
    <form autocomplete="off" class="inline-block margin-left-10">
      <input list="browsers" value="weight_pounds" />
      <datalist id="browsers">
        <option value="weight_pounds" />
        <option value="is_male" />
        <option value="gestation_weeks" />
        <option value="mother_age" />
        <option value="mother_race" />
      </datalist>
      via
      <input list="models" value="natality_model" />
      <datalist id="models">
        <option value="natality_model" />
        <option value="natality_model2" />
        <option value="gestation_weeks" />
      </datalist>
      <input
        v-model="limit"
        class="hidden input-small limit-input"
        type="number"
        autocorrect="off"
...
<script lang="ts">
  ...
  export default class ModelExecutableActions extends Vue {
    @Prop()
    ...
    async execute(): Promise<void> {
      ...
      $.post("/metadata/api/models/predict/ml.natality", {
        "connector": "3",
        "data":
          `weight_pounds,is_male,gestation_weeks,mother_age,mother_race
          8.421658408399999,true,40,40,
          3.31354779786,true,39,28,1
          8.000575487979999,false,40,39,1`
      }, function(data) {

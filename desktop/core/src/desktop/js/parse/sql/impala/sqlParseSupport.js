...
const initSqlParser = function(parser) {
  ...
  parser.commitLocations = function() {
    ...
    let tablePrimaries = [];
    getCleanImpalaPrimaries(parser.yy.latestTablePrimaries).forEach(primary => {
      let cleanPrimary = primary;
      if (primary.identifierChain && primary.identifierChain.length > 0) {
        for (let j = parser.yy.primariesStack.length - 1; j >= 0; j--) {
          getCleanImpalaPrimaries(parser.yy.primariesStack[j]).every(parentPrimary => {
            if (parentPrimary.alias && parentPrimary.alias === primary.identifierChain[0].name) {
              const identifierChain = primary.identifierChain.concat();
    ...
    while (i--) {
      ...
      if (
        ...
        let allPrimaries = tablePrimaries;
        parser.yy.primariesStack.forEach(parentPrimaries => {
          allPrimaries = getCleanImpalaPrimaries(parentPrimaries).concat(allPrimaries);
        });
        const found = allPrimaries.filter(primary => {
      ...
      if (
        ...
        const foundAlias = allPrimaries.filter(primary => {
          return equalIgnoreCase(primary.alias, location.identifierChain[0].name);
        });
  ...
  parser.expandImpalaIdentifierChain = function(tablePrimaries, identifierChain) {
    const expandedChain = identifierChain.concat(); // Clone in case it's called multiple times.
    if (typeof expandedChain === 'undefined' || expandedChain.length === 0) {
      return identifierChain;
    }
    const expand = function(identifier, expandedChain) {
      const foundPrimary = tablePrimaries.filter(tablePrimary => {
        let primaryIdentifier = tablePrimary.alias;
        if (
          !primaryIdentifier &&
          tablePrimary.identifierChain &&
          tablePrimary.identifierChain.length > 0
        ) {
          primaryIdentifier =
            tablePrimary.identifierChain[tablePrimary.identifierChain.length - 1].name;
        }
        return equalIgnoreCase(primaryIdentifier, identifier);
      });

      if (foundPrimary.length === 1 && foundPrimary[0].identifierChain) {
        const parentPrimary = tablePrimaries.filter(tablePrimary => {
          return equalIgnoreCase(tablePrimary.alias, foundPrimary[0].identifierChain[0].name);
        });
        if (parentPrimary.length === 1) {
          const keySet = expandedChain[0].keySet;
          const secondPart = expandedChain.slice(1);
          const firstPart = [];
          // Clone to make sure we don't add keySet to the primaries
          foundPrimary[0].identifierChain.forEach(identifier => {
  ...
  parser.expandIdentifierChain = function(options) {
    ...
    const lengthBefore = identifierChain.length;
    ...
    tablePrimaries.forEach(tablePrimary => {
      if (tablePrimary.identifierChain && tablePrimary.identifierChain.length > 0) {
        ...
        if (tablePrimary.alias) {
          ...
          const testForImpalaAlias = [{ name: table.alias }];
          const result = parser.expandImpalaIdentifierChain(tablePrimaries, testForImpalaAlias);
          if (result.length > 1) {

...
const initSqlParser = function(parser) {
      ...
      return {
        ...
        suggestColRefKeywords: {
          ...
          STRING: parser.isImpala()
            ? ['ILIKE', 'IREGEXP', 'LIKE', 'NOT LIKE', 'REGEXP', 'RLIKE']
            : ['LIKE', 'NOT LIKE', 'REGEXP', 'RLIKE']
        }
      };
    }
    if (
  ...
  parser.valueExpressionSuggest = function(oppositeValueExpression, operator) {
    ...
    if (oppositeValueExpression && oppositeValueExpression.types[0] === 'NUMBER') {
      ...
      parser.isImpala() &&
      (typeof operator === 'undefined' || operator === '-' || operator === '+')
    ) {
  ...
  const linkTablePrimaries = function() {
    ...
    if (
      ...
      } else if (parser.isHive() && !parser.yy.result.suggestColumns.linked) {
        // Expand exploded views in the identifier chain
  ...
  parser.parseSql = function(beforeCursor, afterCursor, dialect, debug) {
    ...
    if (!lexerModified) {
      ...
      parser.lexer.setInput = function(input, yy) {
        const lexer = originalSetInput.bind(parser.lexer)(input, yy);
    ...
    if (typeof result.error !== 'undefined' && typeof result.error.expected !== 'undefined') {
      ...
      result.error.expected.forEach(expected => {
        const match = expected.match(/<([a-z]+)>(.*)/);
        if (match !== null) {
          if (
            typeof parser.yy.activeDialect !== 'undefined' &&
            parser.yy.activeDialect === match[1]
          ) {

    ...
    <div class="container-fluid">
      ...
      <div class="nav-collapse">
        <ul class="nav editor-nav">
          <li class="app-header hide" data-bind="visible: true">
            <!-- ko if: editorMode -->
              <a data-bind="hueLink: '${ url('notebook:editor') }?type=' + editorType(), attr: { 'title': editorTypeTitle() + '${ _(' Editor') }'}" class="hue-link-inline">
            ...
            <!-- /ko -->
            <!-- ko ifnot: editorMode -->
              <i class="fa fa-file-text-o app-icon notebook-icon-inline"></i>
                Notebook
          ...
          <li data-bind="visible: directoryUuid"  class="no-horiz-padding muted hide">
          ...
          <li data-bind="visible: parentSavedQueryUuid" class="no-horiz-padding muted hide">
          ...
          <li data-bind="visible: isSaved() && ! isHistory() && ! parentSavedQueryUuid()" class="no-horiz-padding muted hide">
            <a title="${ _('This is a saved query') }"><i class="fa fa-fw fa-file-o"></i></a>
          </li>
          <li data-bind="visible: isSchedulerJobRunning" class="no-horiz-padding muted hide">
            <a title="${ _('Click to open original saved query') }" data-bind="click: function() { $root.openNotebook(parentSavedQueryUuid()) }" class="pointer inactive-action">
              ${ _("Scheduling on") }
            </a>
          </li>
...
<div class="player-toolbar margin-top-10 hide" data-bind="visible: $root.isPresentationMode()">
  <!-- ko if: $root.isPresentationMode() -->
...
<div id="helpModal${ suffix }" class="modal transparent-modal hide help-modal-inline" data-backdrop="true">
...
<div id="combinedContentModal${ suffix }" class="modal hide custom-modal-inline" data-backdrop="true">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-label="${ _('Close') }"><span aria-hidden="true">&times;</span></button>
...
<script type="text/html" id="notebook${ suffix }">
  ...
  % if not is_embeddable:
    ...
    <div class="resizer hide" data-bind="visible: isRightPanelVisible, splitDraggable : { isRightPanel: true, appName: 'notebook', leftPanelVisible: isLeftPanelVisible, rightPanelVisible: isRightPanelVisible, rightPanelAvailable: isRightPanelAvailable, onPosition: function(){ huePubSub.publish('split.draggable.position') } }"><div class="resize-bar">&nbsp;</div></div>
    <!-- /ko -->
    <div class="assist-container right-panel hide" data-bind="visible: isRightPanelVisible() && isRightPanelAvailable()">
    ...
    <div data-bind="delayedOverflow: 'slow', css: resultsKlass">
      <ul class="nav nav-tabs nav-tabs-editor">
        <li data-bind="click: function(){ currentQueryTab('queryHistory'); }, css: {'active': currentQueryTab() == 'queryHistory'}, onClickOutside: function () { if ($parent.historyFilterVisible() && $parent.historyFilter() === '') { $parent.historyFilterVisible(false) } }">
          <a class="inactive-action" data-testid="editor--query-history--tab" class="editor--query-history--tab-inline" href="#queryHistory" data-toggle="tab">${_('Query History')}</a>
        ...
        <li class="margin-right-20" data-bind="click: function(){ currentQueryTab('savedQueries'); }, css: {'active': currentQueryTab() == 'savedQueries'}, onClickOutside: function () { if (queriesFilterVisible() && queriesFilter() === '') { queriesFilterVisible(false) } }">
          <a class="inactive-action" data-testid="editor--saved-queries--tab" class="editor--saved-queries--tab-inline" href="#savedQueries" data-toggle="tab">${_('Saved Queries')}</a>
          <div class="${_('search-the-saved-queries')}-inline" class="inline-block inactive-action pointer visible-on-hover" title="${_('Search the saved queries')}" data-bind="visible: !queriesHasErrors(), click: function(data, e){ queriesFilterVisible(!queriesFilterVisible()); if (queriesFilterVisible()) { window.setTimeout(function(){ $(e.target).parent().siblings('input').focus(); }, 0); } else { queriesFilter('') }}"><i class="snippet-icon fa fa-search"></i></div>
      ...
      <div class="tab-content" style="border: none; overflow-x: hidden">
        <div class="tab-pane" class="queryHistory-inline" id="queryHistory" data-bind="css: {'active': currentQueryTab() == 'queryHistory'}, style: { 'height' : $parent.historyInitialHeight() > 0 ? Math.max($parent.historyInitialHeight(), 40) + 'px' : '' }">
        ...
        <div class="tab-pane" id="savedQueries" data-bind="css: {'active': currentQueryTab() == 'savedQueries'}" class="savedQueries-inline">
        ...
        <div class="tab-pane margin-top-10" id="queryBuilderTab" data-bind="css: {'active': currentQueryTab() == 'queryBuilderTab'}">
          <div id="queryBuilderAlert" class="alert hide">${ _('There are currently no rules defined. To get started, right click on any table column in the SQL Assist panel.') }</div>
        ...
        <!-- ko if: HAS_WORKLOAD_ANALYTICS && type() === 'impala' -->
        <div class="tab-pane" id="executionAnalysis" data-bind="css: {'active': currentQueryTab() == 'executionAnalysis'}" class="executionAnalysis-inline">
...
<script type="text/html" id="code-editor-snippet-body${ suffix }">
  ...
  <div class="row-fluid editor-row-inline">
    ...
    <div class="editor span12" data-bind="css: {'single-snippet-editor ace-container-resizable' : $root.editorMode() }, clickForAceFocus: ace, visible: ! $root.isResultFullScreenMode() && ! ($root.isPresentationMode() && $root.isHidingCode())">
      <!-- ko if: statementType() == 'file' -->
        <div class="margin-top-10">
          <label class="pull-left query-label-inline">${_('Query File')}</label>
          ...
          <!-- ko if: statementPath() -->
          <div class="inline-block open-link-inline">
...
<script type="text/html" id="notebook-actions">
  <div class="btn-group">
    ...
    <!-- ko with: $root.selectedNotebook() -->
      ...
      <!-- /ko -->
      <!-- ko if: snippets()[executingAllIndex()] && snippets()[executingAllIndex()].isCanceling() -->
      <a class="btn" class="canceling-operation-inline" title="${ _('Canceling operation...') }">
        ...
        <tr data-bind="visible: result.isMetaFilterVisible">
          <td colspan="3">
            <div class="context-popover-inline-autocomplete" style="display: block;">
              ...
              ##<input class="meta-filter" type="text" data-bind="blurHide: result.isMetaFilterVisible, clearable: result.metaFilter, valueUpdate:'afterkeydown'" placeholder="${ _('Filter columns...') }" title="${ _('Type column:xxx or type:yyy for specific filters.') }" class="type-column:xxx-or-type:yyy-for-specific-filters.-inline" />
            </div>
          </td>
        </tr>
      </thead>
      ...
      <div class="row-fluid table-results" data-bind="visible: result.type() == 'table'" style="display: none; max-height: 400px; min-height: 290px;">
        <div>
          ...
          <div class="grid-side" data-bind="css: {'span9': isResultSettingsVisible, 'span12 nomargin': ! isResultSettingsVisible() }">
            <div data-bind="visible: showGrid, delayedOverflow: 'slow', css: resultsKlass" class="hide">
              ...
              <div data-bind="visible: status() == 'expired' && result.data() && result.data().length > 99, css: resultsKlass" class="hide">
                <pre class="margin-top-10"><i class="fa fa-check muted"></i> ${ _("Results have expired, rerun the query if needed.") }</pre>
              </div>
            </div>

            <div data-bind="visible: showChart" class="chart-container hide">
...
<script type ="text/html" id="snippet-execution-controls${ suffix }">
  <div class="snippet-actions" style="position: absolute; bottom: 0">
    ...
    <a class="snippet-side-btn blue" data-testid="editor--execute--button" class="editor--execute--button-inline" title="${ _('Creating session') }">
    ...
    <!-- /ko -->
    <!-- ko if: isCanceling() -->
    <a class="snippet-side-btn" class="canceling-operation-inline" title="${ _('Canceling operation...') }">
      <i class="fa fa-fw fa-spinner snippet-side-single fa-spin"></i>
    </a>
    <!-- /ko -->
    ...
    <div class="inactive-action dropdown hover-actions pointer" data-bind="css: {'disabled': ! isReady() || status() === 'running' || status() === 'loading' }">
      <a class="snippet-side-btn" class="explain-the-current-sql-query-inline" href="javascript: void(0)" data-bind="click: explain, css: {'disabled': ! isReady() || status() === 'running' || status() === 'loading', 'blue': currentQueryTab() == 'queryExplain' }" title="${ _('Explain the current SQL query') }">
